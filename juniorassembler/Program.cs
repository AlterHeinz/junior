...
namespace juniorassembler
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            SymbolMap symbolMap = SymbolMap.OfBank1;
            if (startAddr.Length == 6)
            {
                startAddr = startAddr.Substring(0, 4);
                symbolMap = SymbolMap.OfBank2;
            }
                
            bool withoutFilename = args.Length == 1 || (verbose && args.Length == 2);
            if (withoutFilename)
            {
                var filterPipeline = new HexCharCombiner(new OperationCombiner(new OutputFormatter(Console.Out, verbose, startAddr, symbolMap)));
                Iterators.IterateTextChars(Console.In, filterPipeline);
            }
            else
            {
                string filename = verbose ? args[2]: args[1];
                var filterPipeline = new OperationCombiner(new OutputFormatter(Console.Out, verbose, startAddr, symbolMap));
